...
void BenchmarkDebugPanel::render(Engine::ApplicationContext& context,
                                 const filesystem::path& output_directory)
{
    ...
    render_benchmark_settings(context);
    render_benchmark_result(context);
}

void BenchmarkDebugPanel::render_experiment_controls(Engine::ApplicationContext& context,
                                                     const filesystem::path& output_directory)
{
    ...
    ImGui::Begin(language == Engine::DebugUiLanguage::Korean ? "실험 설정###Experiment Controls"
    ...
    if (ImGui::Button(m_camera_input_enabled ? ui_text(language, "Camera Input: Enabled", "카메라 입력: 활성")
    ...
    ImGui::TextDisabled(
        "%s", ui_text(language, "Camera input is also blocked automatically while ImGui captures input.",
    ...
    if (ImGui::Button(ui_text(language, "Crossing Geometry", "교차 지오메트리"), {scene_button_width, 0.0f}))
    {
    ...
    ImGui::SameLine();
    if (ImGui::Button(ui_text(language, "Particle Stress", "파티클 스트레스"), {scene_button_width, 0.0f}))
    {
    ...
    if (scene_manager.active_scene_type() == Engine::SceneType::EffectStress)
    {
        ...
        if (stress_scene != nullptr)
        {
            ...
            if (ImGui::RadioButton(ui_text(language, "Forced sorting failure", "강제 정렬 실패"),
                                   sorting_failure))
            {
            ...
            ImGui::SameLine();
            if (ImGui::RadioButton(ui_text(language, "Natural volume", "자연스러운 볼륨"), !sorting_failure))
            {
                stress_scene->set_pattern(ParticleStressPattern::NaturalVolume);
                debug_ui.log(Engine::DebugLogLevel::Info, "Particle preset: deterministic natural volume.");
            }
            ImGui::TextWrapped(
                "%s", ui_text(language,
                              "Forced mode creates intersecting ribbons and two overlapping particle sheets. "
                              "A single object-center sort cannot define a correct order for every pixel.",
                              "강제 모드는 서로 관통하는 리본과 겹친 두 파티클 층을 만듭니다. 하나의 "
                              "오브젝트 중심점 정렬로는 모든 픽셀의 올바른 순서를 정할 수 없습니다."));
        }
    }
    ...
    if (method_card(
            "ZSortedAlpha", method_name(Engine::TransparencyMode::ZSortedAlpha, language),
            ui_text(language,
                    "Back-to-front object-center sort. Adds CPU cost and still fails on intersections.",
                    "오브젝트 중심을 뒤에서 앞으로 정렬합니다. CPU 비용이 생기며 교차 지오메트리에는 여전히 "
                    "실패합니다."),
            settings.transparency_mode == Engine::TransparencyMode::ZSortedAlpha, {0.38f, 0.68f, 0.96f, 1.0f},
            language))
    {
    ...
    if (method_card("Wboit", method_name(Engine::TransparencyMode::Wboit, language),
                    ui_text(language,
                            "No CPU sort. Extra accumulation targets and resolve pass increase GPU cost.",
    ...
    const span<const uint32_t> counts = scene_manager.active_scene_type() == Engine::SceneType::Validation
    ...
    if (ImGui::Checkbox(ui_text(language, "Reverse submission order", "제출 순서 뒤집기"), &reverse_order))
    {
    ...
    if (ImGui::Checkbox(ui_text(language, "Auto-flip order every 0.75 s", "0.75초마다 제출 순서 자동 반전"),
                        &m_auto_flip_order))
    {
        m_auto_flip_elapsed = 0.0f;
        debug_ui.log(Engine::DebugLogLevel::Info,
    ...
    ImGui::TextWrapped("%s",
                       ui_text(language,
                               "Quick check: enable Forced sorting failure and Auto-flip. Unsorted Alpha "
                               "should change visibly. Z-Sorted Alpha improves many particles but cannot "
                               "solve intersecting geometry. WBOIT should remain nearly order-stable.",
                               "빠른 확인: 강제 정렬 실패와 자동 반전을 켜세요. 미정렬 알파는 화면이 눈에 "
                               "띄게 변합니다. Z 정렬은 일반 파티클을 개선하지만 교차 지오메트리를 해결하지 "
                               "못합니다. WBOIT는 제출 순서가 바뀌어도 거의 안정적으로 유지됩니다."));
    ImGui::EndDisabled();
    ...
    if (locked)
    {
        ImGui::TextColored({0.96f, 0.72f, 0.20f, 1.0f}, "%s", to_utf8(benchmark.status_text()).c_str());
    }
    else if (ImGui::Button(ui_text(language, "Run Full Benchmark", "전체 벤치마크 실행"), {-1.0f, 0.0f}))
    {
        m_auto_flip_order = false;
        m_open_benchmark_settings = true;
    }

    if (!locked &&
        ImGui::Button(ui_text(language, "Capture Current Frame", "현재 프레임 캡처"), {-1.0f, 0.0f}))
    {
        const auto& active_scene = scene_manager.active_scene();
        wostringstream file_name;
        file_name << (scene_manager.active_scene_type() == Engine::SceneType::Validation ? L"validation_"
                                                                                         : L"stress_")
                  << active_scene.instance_count() << L'_' << method_file_name(settings.transparency_mode)
                  << (settings.reverse_submission_order ? L"_reverse" : L"_forward") << L"_scene.bmp";
...
void BenchmarkDebugPanel::render_performance(Engine::ApplicationContext& context)
{
    ...
    ImGui::Begin(language == Engine::DebugUiLanguage::Korean ? "실시간 성능###Live Performance"
    ...
    ImGui::Text("%s: %s", ui_text(language, "Method", "처리 방식"), method_name(mode, language));
    ImGui::Text("%s: %u", ui_text(language, "Instances", "인스턴스 수"), scene.instance_count());
    ImGui::Text("%s: %u x %u", ui_text(language, "Scene View", "장면 화면"), context.renderer().scene_width(),
                context.renderer().scene_height());
    ImGui::Separator();
    ...
    ImGui::Text(ui_text(language, "Draw calls: %u total, %u transparent", "드로우 콜: 전체 %u, 투명도 %u"),
    ...
    if (mode == Engine::TransparencyMode::Wboit)
    {
        ...
        ImGui::TextWrapped("%s",
                           ui_text(language,
                                   "CPU object sorting is removed. GPU cost rises because transparency "
                                   "writes two accumulation targets and runs a fullscreen resolve pass.",
                                   "CPU 오브젝트 정렬은 사라집니다. 투명도를 두 누적 렌더 타깃에 기록하고 "
                                   "전체 화면 합성 패스를 실행하므로 GPU 비용은 증가합니다."));
    }
    else if (mode == Engine::TransparencyMode::ZSortedAlpha)
    {
        ImGui::TextColored({0.38f, 0.68f, 0.96f, 1.0f}, "%s",
                           ui_text(language, "CPU/GPU tradeoff", "CPU/GPU 트레이드오프"));
        ImGui::TextWrapped("%s", ui_text(language,
                                         "The GPU path is simple alpha blending, but the CPU performs a "
                                         "stable object-center sort every frame.",
                                         "GPU 경로는 단순한 알파 블렌딩이지만 CPU가 매 프레임 안정적인 "
                                         "오브젝트 중심점 정렬을 수행합니다."));
    }
    else
    {
    ...
    ImGui::TextWrapped("%s",
                       ui_text(language,
                               "GPU timestamps cover sky, terrain and transparency. ImGui and Present are "
                               "excluded so the debug tool does not contaminate the benchmark result.",
                               "GPU 타임스탬프는 하늘, 지형과 투명도 렌더링을 포함합니다. 디버그 도구가 "
                               "결과를 오염시키지 않도록 ImGui와 Present는 측정에서 제외합니다."));
    ImGui::End();
}

void BenchmarkDebugPanel::render_benchmark_settings(Engine::ApplicationContext& context)
{
    auto& benchmark = context.benchmark_manager();
    const auto language = context.debug_ui_manager().language();
    auto text = [&](const char* en, const char* ko) { return ui_text(language, en, ko); };
    if (m_open_benchmark_settings)
    {
        ImGui::OpenPopup("Settings###WboitSetup");
        m_open_benchmark_settings = false;
    }
    if (ImGui::BeginPopupModal(text("Settings###WboitSetup", "벤치마크 설정###WboitSetup"), nullptr,
                               ImGuiWindowFlags_AlwaysAutoResize))
    {
        auto& o = benchmark.settings();
        ImGui::Combo(text("Test", "테스트"), &o.test,
                     text("Current scene\0Count sweep\0", "현재 장면\0개수 단계 증가\0"));
        ImGui::Combo(text("Camera", "카메라"), &o.camera,
                     text("Fixed\0Orbit\0Path\0", "고정\0공전\0경로 이동\0"));
        ImGui::SliderInt(text("Maximum instances", "최대 인스턴스"), &o.maximum_instances, 64, 16384);
        ...
        ImGui::InputInt(text("Seed", "랜덤 시드"), &o.seed);
        ImGui::SliderFloat(text("Motion speed", "이동 속도"), &o.motion_speed, 0.1f, 3);
        ImGui::Checkbox(text("Compare all transparency methods", "모든 투명도 방식 비교"),
                        &o.compare_methods);
        ImGui::Checkbox(text("Move stress particles", "스트레스 입자 이동"), &o.animate_effects);
        ImGui::Checkbox(text("Reverse submission order", "제출 순서 반전"), &o.reverse_order);
        ImGui::TextWrapped(
            "%s", text("Fixed simulation step: 1/60 s. Every phase uses the same camera path and seed.",
                       "고정 시뮬레이션 간격: 1/60초. 모든 단계에 같은 카메라 경로와 시드를 사용합니다."));
    ...
    {
        ImGui::Begin(text("Progress###WboitProgress", "벤치마크 진행###WboitProgress"));
        ImGui::TextUnformatted(to_utf8(benchmark.status_text()).c_str());
        if (ImGui::Button(text("Cancel benchmark", "벤치마크 취소")))
            benchmark.cancel(context);
        ImGui::End();
    }
}
...
void BenchmarkDebugPanel::render_benchmark_result(Engine::ApplicationContext& context)
{
    ...
    {
        ImGui::OpenPopup("Results###WboitResults");
        m_open_benchmark_result = false;
    }
    ImGui::SetNextWindowSize({1040, 460}, ImGuiCond_Appearing);
    if (!ImGui::BeginPopupModal(text("Results###WboitResults", "벤치마크 결과###WboitResults"), nullptr))
        return;
    const auto& error = context.benchmark_manager().last_error();
    ImGui::TextWrapped(
        "%s", error.empty()
                  ? text("Completed. All phases are listed below; CSV saved in reports/local/windowed.",
                         "측정 완료. 모든 단계 결과를 표시합니다. CSV: reports/local/windowed")
                  : error.c_str());
    if (ImGui::BeginTable("WboitResults", 8,
                          ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY,
                          {-1, 320}))
    {
        for (const char* title :
             {text("Scene", "장면"), text("Method", "방식"), text("Count", "개수"), text("Run", "반복"),
              "GPU avg ms", "GPU p95 ms", "GPU max ms", "CPU sort ms"})
            ImGui::TableSetupColumn(title);
        ImGui::TableHeadersRow();
        for (const auto& row : context.benchmark_manager().result_summaries())
        {
            ImGui::TableNextRow();
            ImGui::TableNextColumn();
            ImGui::TextUnformatted(scene_name(row.scene, language));
            ImGui::TableNextColumn();
            ImGui::TextUnformatted(method_name(row.mode, language));
            ImGui::TableNextColumn();
            ImGui::Text("%u", row.instance_count);
            ImGui::TableNextColumn();
            ImGui::Text("%u", row.repeat + 1);
            for (double value :
                 {row.gpu_total_mean_ms, row.gpu_total_p95_ms, row.gpu_max_ms, row.cpu_sort_mean_ms})
            {
                ImGui::TableNextColumn();
                ImGui::Text("%.4f", value);
            }
